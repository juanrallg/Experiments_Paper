...
%testing is similar to the one used in previous numerical studies~\cite{SpacePlanes_paper2015,AFMCpaper2014,JSASS_paper,Llobet_PlumeElongation}.
...
\subfloat[Detail of jet and heat transfer gauge locations.]{
...
The numerical domain spans from $\SI{10}{\milli\meter}$ upstream to $\SI{300}{\milli\meter}$ downstream of the fin leading edge and $\SI{200}{\milli\meter}$ centered around the injector centroid in the spanwise direction.
...
The exit profile from this simulation is used as the inflow  boundary condition for for the three-dimensional numerical domain described above.

The structured three-dimensional domain contains approximately four million cells and has a minimum spacing in the vicinity of the injector of approximately $\SI{0.05}{\milli\meter}$.
The cell growth from the injector is restricted to $\SI{1}{\milli\meter}$ in the region of uniform flow and is considered appropriate to resolve the far-field of the vortex-injection interaction.
In the numerical domain, the injector is placed at $X=\SI{125}{\milli\meter}$, downstream from the fin leading edge and 26.2 and 35.2 fin-to-injector distances ratios from the fin in order to match the experiments.
The walls are modelled as non-slip isothermal walls with a temperature of $\SI{300}{\kelvin}$, the mean for the lab in which the experiments were conducted. 
...
%and no boundary layer parabolic stability transition prediction was used. 
...
% KDB: I think you need the qualifying in general statement here as in particular Line A for the  UF gets less acurate as you move away from the fin. More precise but less accurate.
...
In Line A, the left hand side peak is clearly overestimated due to its proximity to the fin and in Line E, the effect of the injection bow shock is largely dissipated, again increasing the relative effect of the `numerical overestimation zone'.
