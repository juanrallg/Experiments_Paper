...
The $\SI{1}{\milli\meter}$ diameter injector was \textbf{initially} located $\SI{126}{\milli\meter}$ downstream of the fin leading edge at a $\SI{45}{\deg}$ angle relative to the freestream flow.
...
The fin can be translated in the model x axis to allow different fin-to-injector distance ratios.
Shown in Fig. \textbf{blah}, by varying this translation distance (\textbf{min-max}), the relative injector position in the Y/X axis changes, allowing for different injection locations in the vortex to be tested. 
...
These gauges are made using an $\approx$\SI{20}{\nano\meter} nickel strip resistive element that is sputtered onto an optically smooth quartz substrate.
...
Shown in Fig.~\ref{fig:GaugesonModel}, in order to appropriately resolve the heat-transfer profile across the injection-vortex interaction, the gauges had to be mounted in close proximity.
